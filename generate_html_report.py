...
parts.append("  <title>Accessibility Audit Report</title>")
parts.append("  <style>")
...
parts.append("    <h1>Accessibility Audit Report</h1>")
parts.append('    <section class="report-details" aria-label="Testing details">')
parts.append('      <p><strong>Page URL:</strong> https://broken-workshop.dequelabs.com/</p>')
    ...
    if not issues:
        ...
        for idx, issue in enumerate(issues, start=1):
            ...
            if urls:
                ...
                for url in urls:
                    parts.append(f'            <img src="{esc(url)}" alt="Issue screenshot" loading="lazy" />')
                parts.append("          </div>")
            else:
                parts.append('          <div class="empty">No screenshot URL provided</div>')
            parts.append('          <span class="label">Source Code</span>')
            parts.append(f'          <pre class="source-code">{source_code or "Not provided"}</pre>')
